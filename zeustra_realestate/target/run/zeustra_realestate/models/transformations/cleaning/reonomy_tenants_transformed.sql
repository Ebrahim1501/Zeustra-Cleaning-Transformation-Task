  ...
  create  table "Zeustra"."public"."reonomy_tenants_transformed__dbt_tmp"
...
CASE WHEN TRIM(LOWER(employee_total))='n/a' OR TRIM(LOWER(employee_total))::int < 0 THEN NULL ELSE  TRIM(employee_total)::int END AS employee_total,
...
SELECT concat('reonomy','|',md5(concat(source_property_id,coalesce(company_name,company_name_2),naics_code,sic_code))) as id ,* FROM dedupl
  );
